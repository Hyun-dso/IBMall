...
import { ensurePostcodeScript } from '@/lib/daumPostcode';
import { signup } from '@/lib/api/members.client';
import { isSuccess } from '@/types/api';
import { emptyToNull } from '@/lib/transform';
import {
    INPUT_CLASS,
    INPUT_GROUP_CLASS,
    INPUT_DIVIDER_CLASS,
...
export default function SignupPage() {
    ...
    const onBlurConfirm = () => {
        // 비번 확인 포커스 해제 시, 토스트 두 번 현상 때문에 일단 비활성화 했음.
        // const msg = validateField('signup', 'confirmPassword', form);
        // if (msg) showToast.error(msg, { group: 'auth.signup' });
    };
    ...
    const handleSearchAddress = async () => {
        ...
        new (window as any).daum.Postcode({
            oncomplete: (data: any) => {
                setField('address1', String(data.address || ''));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await signup({
                ...
                address: emptyToNull(`${form.address1} ${form.address2}`),
            });

            if (!isSuccess(result)) {
                showToast.error(result.message || '회원가입에 실패했어요');
                return;
            }
            ...
            const message = err instanceof Error ? err.message : '네트워크 오류가 발생했어요.';
