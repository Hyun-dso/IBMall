...
export default function GuestPaymentPage() {
    ...
    (async () => {
      try {
        ...
        setProduct({
            ...
            ...(typeof data.timeSalePrice === 'number' ? { timeSalePrice: data.timeSalePrice } : {}),
            ...(data.optionName ? { optionName: data.optionName as string } : {}),
        });
      } catch {
        toast.error('에러 발생');
      } finally {
  ...
  // ✅ 테스트 단계: 서버 계산(정가)와 맞추기 위해 정가 고정
  const unitPrice = useMemo(() => (product ? product.price : 0), [product]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!apiBase) {
      toast.error('백엔드 API base URL 누락');
      return;
    }
    ...
    try {
      ...
      // ✅ 서버 검증 및 저장 (DTO에 맞게 전체 필드 전달)
      try {
        const orderUid = `GUEST-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        const payload = {
          // 검증/멱등
          paymentId: payment.paymentId,
          transactionId: payment.paymentId, // txId 없음 → paymentId 재사용
          paidAmount: amount, // 정가 × 수량
          ...
          recipientName: form.sendToOther ? (form.recipientName || form.buyerName) : form.buyerName,
          recipientPhone: form.sendToOther ? (form.recipientPhone || form.buyerPhone) : form.buyerPhone,
          recipientAddress: `${form.address1} ${form.address2}`.trim(),
        };

        const res = await fetch(`${apiBase}/api/payments/guest/single`, {
          method: 'POST',
          credentials: 'include',
      ...
      <div className="w-full max-w-md mx-auto space-y-8">
        ...
        <form noValidate onSubmit={handleSubmit} className={`w-full max-w-md p-6 ${CARD_CLASS}`}>
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <Switch checked={form.sendToOther} onChange={(v) => setField('sendToOther', v)} label="다른 사람에게 보내기" />
          </div>
