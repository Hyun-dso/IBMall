...
type GuestOrderDetailResponse = { success: true; data: GuestOrderDetail; message: string | null };
...
export default function TrackOrderPage() {
    ...
    // 쿼리파라미터로 orderUid가 들어온 경우 자동 조회
    useEffect(() => {
        if (qpOrderUid) {
            setOrderUid(qpOrderUid);
        }
    }, [qpOrderUid]);

    const canSearch = useMemo(() => {
        return name.trim().length > 0 && normalizePhone(phone).length >= 7;
    }, [name, phone]);
    ...
    const onSearchList = async () => {
        ...
        try {
            ...
            const r = await fetchJson<GuestOrderListResponse>(`/api/track/guest/orders/search`, {
                method: 'POST',
                credentials: 'include',
            ...
            if (!r?.success) {
                showToast.error('주문 목록 조회에 실패했습니다.');
                return;
            }
            setList(r.data.content);
            if (r.data.content.length === 0) {
                showToast.error('조회된 주문이 없습니다.');
            }
        } catch {
            showToast.error('주문 목록 조회 중 오류가 발생했습니다.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const onFetchDetail = async (uid: string) => {
        ...
        try {
            ...
            const r = await fetchJson<GuestOrderDetailResponse>(
            ...
            if (!r?.success) {
                showToast.error('주문 상세 조회에 실패했습니다.');
                return;
            }
            setDetail(r.data);
            // 완료 페이지로 이동할 때 사용할 힌트 저장
            try {
            ...
            showToast.error('주문 상세 조회 중 오류가 발생했습니다.');
        } finally {
            setLoading(false);
        }
    };
                                    ...
                                    <li key={o.orderUid} className="py-3 flex items-center justify-between">
                                        <div className="min-w-0">
                                            ...
                                            <div className="text-sm text-text-secondary dark:text-dark-text-secondary">
                                                {safeFormat(o.createdAt)} • {o.status} • {currency.format(o.totalPrice)}원
                                            </div>
                    ...
                    <section ref={detailRef} className={`p-6 ${CARD_CLASS}`}>
                        ...
                        <div className="mt-6 flex gap-2">
                            <Button
                                className="flex-1"
                                onClick={() => {
                                    // 완료 페이지로 이동(기본값 복귀 로직 포함)
                                    const url = `/orders/complete?orderUid=${encodeURIComponent(detail.orderUid)}`;
                                    router.push(url);
                                }}
                            >
                                주문 완료 화면으로
