...
export default function ProductBuyButtons(props: Props) {
    ...
    if (props.mode === 'cart') {
        ...
        return (
            <Button className={className} variant="solid" size="md" disabled={disabled} onClick={onBuyNow} aria-label="장바구니 결제">
                {label}
            </Button>
        );
    }
    ...
    return (
        <Button className={className} variant="solid" size="md" disabled={disabled} onClick={handleBuy} aria-label="바로구매">
            {label}
        </Button>
    );
}
